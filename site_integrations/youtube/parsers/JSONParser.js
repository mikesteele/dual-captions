const YouTubeAutogeneratedParser = (parsedCaptionFile) => {
  return parsedCaptionFile.events.map(e => ({
    text: e.segs && e.segs.map(f => f.utf8).join(""),
    startTime: e.tStartMs,
    endTime: e.tStartMs + e.dDurationMs
  })).filter(e => e.text !== '\n');
}
...
const YouTubeJSONParser = (captionFile) => {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      if (isAutogenerated) {
        resolve(YouTubeAutogeneratedParser(parsedCaptionFile));
