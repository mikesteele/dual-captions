...
window.DUAL_CAPTIONS.TKK = '0';

function updateTKK() {
    return new Promise(function (resolve, reject) {
        var now = Math.floor(Date.now() / 3600000);

        if (Number(window.DUAL_CAPTIONS.TKK.split('.')[0]) === now) {
            resolve();
        } else {
...
window.DUAL_CAPTIONS.token = {};
window.DUAL_CAPTIONS.token.get = function(text) {
