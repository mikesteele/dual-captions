class NetflixConfig extends DualCaptionsConfig {
  ...
  _repositionNetflixCaptions(element) {
    ...
    const diff = dualCaptionsWindow.scrollHeight - netflixCaptionsHeight;
    netflixCaptions.forEach(caption => {
      if (caption.hasAttribute('DC_original-top')) {
        caption.style.top = `calc(${caption.getAttribute('DC_original-top')} - ${netflixCaptionsHeight}px - ${diff}px)`;
      } else if (caption.hasAttribute('DC_original-bottom')) {
        caption.style.bottom = `calc(${caption.getAttribute('DC_original-bottom')} + ${netflixCaptionsHeight}px + ${diff}px))`;
      } else {
        if (caption.style.top) {
          const top = caption.style.top;
          caption.setAttribute('DC_original-top', top);
          caption.style.top = `calc(${top} - ${netflixCaptionsHeight}px - ${diff}px)`;
        } else if (caption.style.bottom) {
          const bottom = caption.style.bottom;
          caption.setAttribute('DC_original-bottom', bottom);
          caption.style.bottom = `calc(${bottom} + ${netflixCaptionsHeight}px + ${diff}px)`;
        }
      }
    });
    this._repositionDualCaptionsWindow(diff);
  }

  _repositionDualCaptionsWindow(diff) {
    const dualCaptionsWindow = document.querySelector('.dual-captions-window');
    ...
    if (lastNetflixCaption.hasAttribute('DC_original-top')) {
      ...
      dualCaptionsWindow.style.top = `calc(${firstNetflixCaption.getAttribute('DC_original-top')} - ${diff}px)`;
    } else if (lastNetflixCaption.hasAttribute('DC_original-bottom')) {
      dualCaptionsWindow.style.bottom = `calc((${lastNetflixCaption.getAttribute('DC_original-bottom')} + ${diff}px)`;
    }
