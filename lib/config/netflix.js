class NetflixConfig extends DualCaptionsConfig {
  ...
  styleCaptionElement(element, mutation) {
    ...
    element.style.width = `${realCaption.parentNode.scrollWidth}px`; // TODO
    element.style.display = 'block';
    element.setAttribute('__dc_caption__', true);
    return element;
  }
  ...
  _repositionNetflixCaptions(dcWindow) {
    ...
    const heightDiff = dcWindow.scrollHeight - netflixCaptionsHeight;
    ...
    netflixCaptions.forEach(caption => {
      ...
      if (caption.hasAttribute('__original-top__')) {
        ...
        caption.style.bottom = `calc(${caption.getAttribute('__original-bottom__')} + ${netflixCaptionsHeight}px + ${heightDiff}px)`;
      }
    });
  ...
  _repositionDCWindow(dcWindow, netflixCaptions, heightDiff) {
    ...
    if (lastNetflixCaption.hasAttribute('__original-top__')) {
      ...
      dcWindow.style.bottom = `calc((${lastNetflixCaption.getAttribute('__original-bottom__')} + ${heightDiff}px)`;
    }
  }
}
